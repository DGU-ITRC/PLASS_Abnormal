...
async def inference(video=None):
    ...
    # C3D MODEL
    yield f"data: {json.dumps({'message': f'Initialize C3D Model...'})}\n\n"
    c3d_model = init_c3d_model(args)
    yield f"data: {json.dumps({'message': f'Initialize C3D Model... Done!'})}\n\n"
    clip = []
    cap = cv2.VideoCapture(args['video'])
    num_frame = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    cur_frame = 1
    yield f"data: {json.dumps({'message': f'Preprocessing Video...'})}\n\n"
    while True:
        flag, frame = cap.read()
        if flag:
            progress = math.ceil(cur_frame / num_frame * 100)
            ...
            yield f"data: {json.dumps({'message': f'Preprocessing Video... Done!'})}\n\n"
            break
    
    yield f"data: {json.dumps({'message': f'Inference Action(Model, C3D)...'})}\n\n"
    inference_result_list = []
    last_idx = min(args['limit_frame'], num_frame - 15)
    for start_idx in range(0, num_frame-15):
        progress = math.ceil(start_idx / last_idx * 100)
        yield f"data: {json.dumps({'progress': f'{progress}'})}\n\n"
        end_idx = start_idx + args['inference_frame_num']
        if end_idx < num_frame and start_idx < last_idx:
            label = inference_c3d(c3d_model, clip[start_idx:end_idx])
            ...
            break
    yield f"data: {json.dumps({'message': f'Inference Action(Model, C3D)... Done!'})}\n\n"
    c3d_result = max(set(inference_result_list), key=inference_result_list.count)
    yield f"data: {json.dumps({'message': f'Extracting Video to Frames...'})}\n\n"

    # SEQ MODEL
    yield f"data: {json.dumps({'message': f'Initialize Seq Model...'})}\n\n"
    seq_model = init_seq_model(args)
    yield f"data: {json.dumps({'message': f'Initialize Seq Model... Done!'})}\n\n"
    yield f"data: {json.dumps({'message': f'Extracting Video to Frames...'})}\n\n"
    ...
    if cap.isOpened():
        while True:
            ...
            if ret:
                ...
                break
    cap.release()
    yield f"data: {json.dumps({'message': f'Extracting Video to Frames... Done!'})}\n\n"
    yield f"data: {json.dumps({'message': f'Extracting Frames to Skeletons...'})}\n\n"
    bitmap_frames = extract_skeleton(args, extracted_frames)
    yield f"data: {json.dumps({'message': f'Extracting Frames to Skeletons...Done!'})}\n\n"
    yield f"data: {json.dumps({'message': f'Initialize Feature Extractor...'})}\n\n"
    feature_extractor = init_feature_extractor(args)
    yield f"data: {json.dumps({'message': f'Initialize Feature Extractor... Done!'})}\n\n"
    label_processor = tf.keras.layers.experimental.preprocessing.StringLookup(num_oov_indices=0, vocabulary=[str(key) for key in LABELMAP.keys()])
    vocabulary = label_processor.get_vocabulary()
    yield f"data: {json.dumps({'message': f'Inference Action(Model, SEQ)...'})}\n\n"
    feature, mask = extract_feature_mask(args, feature_extractor, bitmap_frames)
    seq_result = inference_seq(seq_model, vocabulary, feature, mask)
    yield f"data: {json.dumps({'message': f'Inference Action(Model, SEQ)... Done!'})}\n\n"

    # CATEGORIZATION
    yield f"data: {json.dumps({'message': f'Inference Result: C3D Model: {c3d_result}, SEQ Model: {seq_result}'})}\n\n"
    yield f"data: {json.dumps({'message': f'Categorization Reuslts...'})}\n\n"
    c3d_category = categorization(c3d_result)
    seq_category = categorization(seq_result)
    yield f"data: {json.dumps({'message': f'Categorization Reuslts... Done!'})}\n\n"
    yield f"data: {json.dumps({'message': f'Categorized Result: C3D Model: {c3d_result}, SEQ Model: {seq_result}'})}\n\n"

    # COMBINE
    yield f"data: {json.dumps({'message': f'Comine Reuslts...'})}\n\n"
